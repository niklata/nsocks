...
bool compare_ipv6(boost::asio::ip::address_v6::bytes_type ip,
                  boost::asio::ip::address_v6::bytes_type mask,
                  unsigned int msize)
{
    if (msize > 128)
        return false;
...
bool compare_ip(boost::asio::ip::address ip,
                boost::asio::ip::address mask, unsigned int msize)
{
    boost::asio::ip::address_v6
        ip6(ip.is_v4() ? boost::asio::ip::address_v6::v4_mapped(ip.to_v4())
                       : ip.to_v6()), mask6;
    if (mask.is_v4()) {
        mask6 = boost::asio::ip::address_v6::v4_mapped(mask.to_v4());
        msize += 96;
    } else
        mask6 = mask.to_v6();
